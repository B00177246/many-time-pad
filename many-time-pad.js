const crypto = require('crypto');

// Tuple containing hex-encoded cipher texts
const cipherTexts = [
    '0f381a39fe6f41bd57c44646eacc3ecb2b695ae729ee174ac650ab0c92547a73b19ca7a24d40162bea9c0d1c2c1395678f6dec2ae8b4eaa449b1511507c3e06dd6c9c02c69c5eca4241d3f3585f44440ad011078381bacc075e4c3',
    '0f351c71d96e59b742c34053a6853d9930664da237f24456874ae61198546b7ea6c8f7a34b581823ead8490c29568e618b68a929f3e6e6ea0ca35f1944c2ec70d686df3028c4f9a42a0720748cbb4e41a74c07773213bad56eefde922d44cdbcbf3e008be80870a5eb7c55ab907f18fcf347dd433bcf83432d0849e80c22b0',
    '',
    '',
    '',
    '',
    '',
    '',
    '14331c71fd614eba59c34007e58d2099206108f632f81755c851e04198403f79a1daa3a902590d2be6964c1b26138f6b95258228a7abeee744be591944c9e46d828dc2697cc3faa4351d3f7ec2f44b0ea54f17393e08afd366efc2d63743c2ada33e1982e3',
    '',
    '',
    '',
    '0f351c71c7654ff251de056ea68920cf2d7d49e53ff9174bd303fc04d74e7e69ad9cb9bf56160c2aea960d002b139b6b8f25982fe2e6e9f158a2451944c3f623d19dc425648beceb621f38768abb4f47ad46176b7b04b3c069a0c4da3b0dd3bea96a54b7e4453989f86b55ba8b635b90f944'
];


// Function to convert hex string to bytes
const hexToBytes = (hex) => {
    return Buffer.from(hex, 'hex');
};

// Determine the length of the last cipher text
const cipherTextLength = hexToBytes(cipherTexts[cipherTexts.length - 1]).length;

// Convert hex strings to bytes
const cipherBytesList = cipherTexts.map(hexToBytes);

// Initialize an empty string to accumulate the potential plaintext
let potentialPlaintext = '';

// Iterate through each byte of the last cipher text
for (let byteIndex = 0; byteIndex < cipherBytesList[cipherBytesList.length - 1].length; byteIndex++) {
    const byteValue = cipherBytesList[cipherBytesList.length - 1][byteIndex];
    const possiblePlaintextChars = {};

    // Compare with each of the other cipher texts
    for (const cipherBytes of cipherBytesList.slice(0, -1)) {
        // Skip if the current index is out of bounds for the cipher text
        if (byteIndex >= cipherBytes.length) continue;

        // Attempt to decode each byte using possible plaintext characters
        for (const potentialCharByte of Buffer.from('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ ')) {
            // XOR operation to get the potential plaintext character
            const decodedChar = String.fromCharCode(byteValue ^ cipherBytes[byteIndex] ^ potentialCharByte);

            // Only consider valid characters
            if (!/^[a-zA-Z:,\s]$/.test(decodedChar)) continue;

            // Count occurrences of each character
            if (possiblePlaintextChars[decodedChar]) {
                possiblePlaintextChars[decodedChar]++;
            } else {
                possiblePlaintextChars[decodedChar] = 1;
            }
        }
    }

    // Append the most frequently occurring character to the plaintext
    if (Object.keys(possiblePlaintextChars).length > 0) {
        const maxChar = Object.keys(possiblePlaintextChars).reduce((a, b) => possiblePlaintextChars[a] > possiblePlaintextChars[b] ? a : b);
        potentialPlaintext += maxChar;
    }
}

// Print the reconstructed potential plaintext
console.log(potentialPlaintext);
